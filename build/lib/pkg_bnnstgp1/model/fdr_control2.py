...
class BetaFdr:
    ...
    def fdr(self, net, weight_samples, net_phi, net_b, coord):
        ...
        for j in range(beta.shape[2]):
            ...
            while i < len(b1):
                i += 1
                if sum(b1_sort_inv[:i+1])/(i+1) > 0.2:
                    l = i
                    ind = b1_ind[i]
                    delta[j] = i
                    break
    ...
    def beta_fdr(self,idx):
        ...
        # for seed in range(self.rep):
            ...
            b[abs(b)>0] = 1
            # b = np.mean(b,axis=0)
            # pd.DataFrame(b).to_csv("Simulation4_total"+str(idx)+"/Itr"+str(seed)+".csv")
            # pd.DataFrame(b).to_csv("Simulation_total"+str(idx)+"/all.csv")
            # pd.DataFrame(b).to_csv("Re_total"+str(idx)+"/Itr"+str(seed)+".csv")
