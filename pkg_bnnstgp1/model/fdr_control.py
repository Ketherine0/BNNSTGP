...
class BetaFdr:
    ...
    def __init__(self, rep, path, lr, lamb, Y,
                 n_hid, n_hid2, n_hid3, n_hid4, fdr_rate, hid_u=None,coord=None, imgs=None, W=None,  
    ...
    def fdr(self, net, weight_samples, net_phi, net_b, coord, fdr_rate):
        # self.coord = coord
        ...
        for j in range(beta.shape[2]):
            ...
            while i < len(b1):
                i += 1
                if sum(b1_sort_inv[:i+1])/(i+1) > self.fdr_rate:
                    l = i
                    ind = b1_ind[i]
                    delta[j] = i
                    break
    ...
    def beta_fdr(self,idx):
        ...
        print(self.hid_u)
        beta_all = np.zeros((self.rep, self.hid_u, self.coord[idx].shape[0]))
        ...
        for seed in range(self.rep):
            ...
            beta_all[seed,:,:] = self.fdr(net, weight_samples, net.model.phi[idx], net.model.b[idx], self.coord[idx],fdr_rate = self.fdr_rate)
